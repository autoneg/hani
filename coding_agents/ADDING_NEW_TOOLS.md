...
class UtilityHistoryTool(Tool):
    ...
    def action_executed(self, session_state: dict[str, Any], nmi: SAONMI, action):
        ...
        if state.current_offer:
            ...
            if self.show_agent:
                agent_ufun = mechanism.negotiators[
                    1 - session_state["human_index"]
                ].ufun
        ...
        return pn.Column(
            ...
            pn.Row(
                pn.widgets.Checkbox.from_param(
                    self.param.show_agent, name="Show Agent Utility"
                ),
...
# ✅ GOOD: Use .from_param()
slider = pn.widgets.IntSlider.from_param(self.param.my_param, name="My Parameter")
